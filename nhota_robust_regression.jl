...
# ============================================================================
# Dataset: a1a from LIBSVM (small, real-world binary classification)
...
# ============================================================================
# PART 1: Load LIBSVM Dataset (a1a - small real dataset)
# ============================================================================

function download_a1a()
    url = "https://www.csie.ntu.edu.tw/~cjlin/libsvmtools/datasets/binary/a8a"
    ...
    rm(datafile)
    println("Loaded a1a: n=$n samples, d=$d features")
    return A, labels
end
...
function solve_subproblem_p2(xk::Vector{Float64}, grad_F::Vector{Float64}, 
                              H_F::Matrix{Float64}, lambda::Float64, M::Float64)
    d = length(xk)
    reg = M / 2.0
    H_reg = H_F + reg * I
    H_reg = (H_reg + H_reg') / 2
...
function solve_subproblem_p3(xk::Vector{Float64}, grad_F::Vector{Float64}, 
    ...
    for inner in 1:max_inner
        grad_cubic = grad_F + H_F * s + third_deriv_action(xk, s, lambda)
        s_new = s .- grad_cubic ./ M
...
function run_experiments()
    ...
    # Nonconvex regularization parameter
    #############################################################
    #
    #
    #a1a:lambda = 0.001, x0 = rand(rng, d) * 0.0
    #
    #
    #############################################################
    lambda = 0.1
...
function plot_comparison(results::Dict; save_prefix::String="figures/nhota_a1a")
    ...
    # Define styles: u=0.5 methods are transparent with star markers
    # u=1.0 and baseline are solid lines without markers
    styles = Dict(
    ...
    # Helper to get y-range that shows the plateau
    ...
    # Plot 1: Objective vs Iterations (linear scale to see plateau)
    ...
    # Plot 2: Stationarity vs Iterations (log scale)
    ...
    # Plot 3: Objective vs CPU Time (linear scale)
    ...
    # Plot 4: Stationarity vs CPU Time (log scale)
